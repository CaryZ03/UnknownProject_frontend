  ...
  <div class="container">
    <header>
      ...
      <nav class="navigation">
        ...
        <button  class="btnLogin-popup" @click="showJoinTeamDialog">
          JoinTeam
        </button>
        <span v-if="!this.IsLoginFinished">
          <button v-show="!this.$store.state.isLogin" class="btnLogin-popup" @click="login">
          Login
        </button>
    ...
    <div class="wrapper">
      ...
      <div class="form-box login">
        ...
        <h2 v-else>加入团队</h2>
        <form @submit.prevent="" v-if="!IsLoginFinished">
          <div class="input-box">
        ...
        <form @submit.prevent="" v-else>
          <div class="input-box">
            <span class="icon"><i class="el-icon-edit"></i></span>
            <input type="text" v-model="messageToSend" required />
            <label>备注信息</label>
          </div>

          <button @click="SendInvitation" class="btn">发起加入申请</button>
...
export default {
  ...
  methods: {
    ...
    SendInvitation(){
      ...
      this.$api.team.post_join_team_url(tmp).then((response) => {
            // console.log(tmp)
            // console.log(response.data)
            if (response.data.errno == 0) {
              console.log("获取成功")
  ...
  mounted() {
    ...
    // if(this.$store.state.isLogin){
          // this.flashTeamList();
    // }
    let op = localStorage.getItem('isLogin')
    console.log("type:",typeof op)
    console.log("getltem: ",op)
    console.log("!getItem: ",(op===false))
    this.wrapper.classList.add("active-popup");
