<template>
    ...
    <el-aside width="300px" style="border: 1px solid #eee; ">
      <span  class="inherited-styles-for-exported-element">回收站目录</span>
        <div >
                ...
                <span class="custom-tree-node" slot-scope="{ node, data }" style="display: flex;align-items: center;margin-left: 8px;" @click="chooseDocument(node)">
                  ...
                  <el-button
                    ...
                    <el-button
                      v-if="IsRecycleButtonShown"
                      size="mini"
                      
                      circle
                      @click="handleRecycleNode(node)"
    ...
    <el-container >
      ...
      <el-main style="background-color:#fffcd7;">
        <el-descriptions class="margin-top"  :column="1"  style="max-width: 600px;box-shadow: 0px 0px 16px rgba(17, 17, 26, 0.1);" border>
  ...
  <script>
    export default {
      ...
      methods: {
        showRecycleButton(){
            this.IsRecycleButtonShown = !this.IsRecycleButtonShown
            if(this.showDeleteButton)
                this.showDeleteButton = false
        },
       

        handleRecycleNode(node){
        ...
        handleDeleteDocument1(){
            ...
            this.$confirm('确定要删除吗?此次删除后无法恢复', '提示', {
          ...
          const tmp={
              ...
              console.log(" input:",tmp)
              this.$api.document.post_delete_document(tmp).then((response) => {
                // console.log(tmp)
                // console.log(response.data)
                if (response.data.errno == 0) {
                  console.log("获取成功")
      ...
      this.newDirectoryDialogVisible = false;  // 关闭对话框
        ...
        handleDeleteNode(node){
          ...
          this.$confirm('确定要删除吗?此次删除后无法恢复', '提示', {
            ...
            if(node.data.isFolder){  //是文件夹
              ...
              console.log(" input:",tmp)
              this.$api.document.post_delete_directory(tmp).then((response) => {
                // console.log(tmp)
                // console.log(response.data)
                if (response.data.errno == 0) {
                  console.log("获取成功")
        ...
        handleDeleteDocument(){
          ...
          if(this.IsRecycleButtonShown)
            this.IsRecycleButtonShown = false
        },
        ...
        showDirectoryTree(){
            ...
            this.$api.document.post_show_recycle_tree(tmp).then((response) => {
              // console.log(tmp)
              // console.log(response.data)
              if (response.data.errno == 0) {
                console.log("获取成功")
                ...
                this.root_id = response.data.recycle_id;
